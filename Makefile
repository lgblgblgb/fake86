...
CFLAGS_WIN	= -std=c11 -O3 -Wall -pipe -DPATH_DATAFILES=\"$(DATAPATH)/\" -D_DEFAULT_SOURCE
INCLUDE		= -Isrc/fake86
LIBS		= -lpthread -lX11
LIBS_WIN	=
SDLFLAGS	= `sdl2-config --cflags --libs`
SDLFLAGS_WIN	= `x86_64-w64-mingw32-sdl2-config --cflags --libs`
...
$(BIN_FAKE86).exe: $(SRCFILES) $(HEADERS)
	$(CC_WIN) $(SRCFILES) -o $@ $(CFLAGS_WIN) $(INCLUDE) $(LIBS_WIN) $(SDLFLAGS_WIN)
...
clean:
	rm -f src/fake86/*.o src/imagegen/*.o $(BINS) $(BIN_FAKE86).exe $(BIN_IMAGEGEN).exe
